...
module {
  ...
  func.func @main(%arg0: tensor<1x1024x768xf32>) -> tensor<1x1024x3072xf32> {
    ...
    //^bb0(%in: f32, %out: f32):
      //linalg.yield %in : f32
    //} -> tensor<768x768xf32>
    %1 = tensor.empty() : tensor<768x3072xf32>
    %2 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel"]} ins(%cst_6 : tensor<3072x768xf32>) outs(%1 : tensor<768x3072xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    } -> tensor<768x3072xf32>
    %3 = tensor.empty() : tensor<1x1024x3072xf32>
    %4 = linalg.generic {indexing_maps = [#map8, #map9, #map10], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%arg0, %2 : tensor<1x1024x768xf32>, tensor<768x3072xf32>) outs(%3 : tensor<1x1024x3072xf32>) {
    ^bb0(%in: f32, %in_6: f32, %out: f32):
      %26 = arith.mulf %in, %in_6 : f32
      %27 = arith.addf %26, %out : f32
      linalg.yield %27 : f32
    } -> tensor<1x1024x3072xf32>
    return %4 : tensor<1x1024x3072xf32>
  }
 }