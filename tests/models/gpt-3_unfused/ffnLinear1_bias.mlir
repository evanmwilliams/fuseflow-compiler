...
module {
  ...
  func.func @main(%arg0: tensor<1x1024x3072xf32>) -> tensor<1x1024x3072xf32> {
    ...
    %0 = tensor.empty() : tensor<1x1024x3072xf32>
    %1 = linalg.generic {indexing_maps = [#map4, #map7, #map4], iterator_types = ["parallel", "parallel", "parallel"]} ins(%arg0, %cst_7 : tensor<1x1024x3072xf32>, tensor<3072xf32>) outs(%0 : tensor<1x1024x3072xf32>) {
        ^bb0(%in: f32, %in_18: f32, %out: f32):
          %57 = arith.addf %in, %in_18 : f32
          linalg.yield %57 : f32
        } -> tensor<1x1024x3072xf32>
    return %1 : tensor<1x1024x3072xf32>
  }
 }