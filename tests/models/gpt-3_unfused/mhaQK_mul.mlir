...
module attributes {torch.debug_module_name = "MultiHeadAttention"} {
  ...
  func.func @forward(%arg1: tensor<1x12x1024x64xf32, #SPARSE>, %arg2: tensor<1x12x64x1024xf32, #SPARSE>, %arg3: tensor<1x12x1024x64xf32, #SPARSE>) -> tensor<1x12x1024x1024xf32, #SPARSE> {
    ...
    %23 = linalg.generic {indexing_maps = [#map8, #map9, #map10], iterator_types = ["parallel", "parallel", "parallel", "reduction", "parallel"]} ins(%arg1, %arg2 : tensor<1x12x1024x64xf32, #SPARSE>, tensor<1x12x64x1024xf32, #SPARSE>) outs(%22 : tensor<1x12x1024x1024xf32, #SPARSE>) {
    ^bb0(%in: f32, %in_12: f32, %out: f32):
      %43 = arith.mulf %in, %in_12 : f32
      %44 = arith.addf %43, %out : f32
      linalg.yield %44 : f32
    } -> tensor<1x12x1024x1024xf32, #SPARSE>
    %24 = linalg.generic {indexing_maps = [#map5, #map5], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%23 : tensor<1x12x1024x1024xf32, #SPARSE>) outs(%21 : tensor<1x12x1024x1024xf32, #SPARSE>) {
    ^bb0(%in: f32, %out: f32):
    ...
    %36 = linalg.generic {indexing_maps = [#map8, #map9, #map10], iterator_types = ["parallel", "parallel", "parallel", "reduction", "parallel"]} ins(%34, %arg3 : tensor<1x12x1024x1024xf32, #SPARSE>, tensor<1x12x1024x64xf32, #SPARSE>) outs(%35 : tensor<1x12x1024x64xf32, #SPARSE>) {
    ^bb0(%in: f32, %in_12: f32, %out: f32):
      %43 = arith.mulf %in, %in_12 : f32
      %44 = arith.addf %43, %out : f32
      linalg.yield %44 : f32
    } -> tensor<1x12x1024x64xf32, #SPARSE>
    return %23 : tensor<1x12x1024x1024xf32, #SPARSE>
  }
}