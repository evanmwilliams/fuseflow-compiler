...
module attributes {torch.debug_module_name = "MultiHeadAttention"} {
  ...
  func.func @forward(%arg1: tensor<1x12x4096x4096xf32, #SPARSE>, %arg2: tensor<1x12x64x4096xf32, #SPARSE>, %arg3: tensor<1x12x4096x64xf32, #SPARSE>) -> tensor<1x12x4096x4096xf32, #SPARSE> {
    ...
    %29:2 = linalg.generic {indexing_maps = [#map5, #map11, #map11], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%arg1 : tensor<1x12x4096x4096xf32, #SPARSE>) outs(%28, %26 : tensor<1x12x4096x1xf32, #SPARSE>, tensor<1x12x4096x1xi64>) {
    ...
    } -> (tensor<1x12x4096x1xf32, #SPARSE>, tensor<1x12x4096x1xi64>)
    %30 = linalg.generic {indexing_maps = [#map5, #map11, #map5], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%arg1, %29#0 : tensor<1x12x4096x4096xf32, #SPARSE>, tensor<1x12x4096x1xf32, #SPARSE>) outs(%21 : tensor<1x12x4096x4096xf32, #SPARSE>) {
    ^bb0(%in: f32, %in_12: f32, %out: f32):
      %43 = arith.subf %in, %in_12 : f32
      linalg.yield %43 : f32
    } -> tensor<1x12x4096x4096xf32, #SPARSE>
    %31 = linalg.generic {indexing_maps = [#map5, #map5], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%30 : tensor<1x12x4096x4096xf32, #SPARSE>) outs(%21 : tensor<1x12x4096x4096xf32, #SPARSE>) {
    ^bb0(%in: f32, %out: f32):
      %43 = math.exp %in : f32
      linalg.yield %43 : f32
    } -> tensor<1x12x4096x4096xf32, #SPARSE>
    %32 = linalg.fill ins(%cst_7 : f32) outs(%27 : tensor<1x12x4096x1xf32, #SPARSE>) -> tensor<1x12x4096x1xf32, #SPARSE>
    %33 = linalg.generic {indexing_maps = [#map5, #map11], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%31 : tensor<1x12x4096x4096xf32, #SPARSE>) outs(%32 : tensor<1x12x4096x1xf32, #SPARSE>) {
    ^bb0(%in: f32, %out: f32):
      %43 = arith.addf %in, %out : f32
      linalg.yield %43 : f32
    } -> tensor<1x12x4096x1xf32, #SPARSE>
    %34 = linalg.generic {indexing_maps = [#map5, #map11, #map5], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%31, %33 : tensor<1x12x4096x4096xf32, #SPARSE>, tensor<1x12x4096x1xf32, #SPARSE>) outs(%21 : tensor<1x12x4096x4096xf32, #SPARSE>) {
    ^bb0(%in: f32, %in_12: f32, %out: f32):
      %43 = arith.divf %in, %in_12 : f32
      linalg.yield %43 : f32
    } -> tensor<1x12x4096x4096xf32, #SPARSE>
    %35 = tensor.empty() : tensor<1x12x4096x64xf32, #SPARSE>
    %36 = linalg.generic {indexing_maps = [#map8, #map9, #map10], iterator_types = ["parallel", "parallel", "parallel", "reduction", "parallel"]} ins(%34, %arg3 : tensor<1x12x4096x4096xf32, #SPARSE>, tensor<1x12x4096x64xf32, #SPARSE>) outs(%35 : tensor<1x12x4096x64xf32, #SPARSE>) {
    ^bb0(%in: f32, %in_12: f32, %out: f32):
      %43 = arith.mulf %in, %in_12 : f32
      %44 = arith.addf %43, %out : f32
      linalg.yield %44 : f32
    } -> tensor<1x12x4096x64xf32, #SPARSE>
    return %34 : tensor<1x12x4096x4096xf32, #SPARSE>
  }
}
