...
module {
  ...
  func.func @main(%arg0: tensor<1x1024x768xf32>) -> tensor<1x1024x768xf32> {
    ...
    // %4 = linalg.matmul ins(%collapsed, %cst : tensor<1024x768xf32>, tensor<768x768xf32>) outs(%3 : tensor<1024x768xf32>) -> tensor<1024x768xf32>
    %4 = linalg.generic {indexing_maps = [#map8, #map9, #map10], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%arg0, %cst : tensor<1x1024x768xf32>, tensor<768x768xf32>) outs(%2 : tensor<1x1024x768xf32>) {
