...
module attributes {torch.debug_module_name = "GCN"} {
  ...
  // func.func @forward(%arg0: tensor<1767x16xf32>, %arg1: tensor<1767x1767xf32, #SPARSE>) -> tensor<1767x121xf32, #SPARSE> {
  func.func @forward(%arg0: tensor<1767x16xf32>, %arg1: tensor<1767x1767xf32, #SPARSE>) -> tensor<1767x16xf32, #SPARSE> {
    ...
    // %4 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel"]} ins(%cst : tensor<16x50xf32, #SPARSE>) outs(%3 : tensor<50x16xf32>) {
    // ^bb0(%in: f32, %out: f32):
    //   linalg.yield %in : f32
    // } -> tensor<50x16xf32>
    ...
    // %7 = linalg.matmul ins(%2, %4 : tensor<1767x50xf32>, tensor<50x16xf32>) outs(%6 : tensor<1767x16xf32, #SPARSE>) -> tensor<1767x16xf32, #SPARSE>
    ...
    %9 = linalg.generic {indexing_maps = [#map, #map], iterator_types = ["parallel", "parallel"]} ins(%arg0 : tensor<1767x16xf32>) outs(%6 : tensor<1767x16xf32, #SPARSE>) {
    ^bb0(%in: f32, %out: f32):
      %29 = arith.cmpf ugt, %in, %cst_0 : f32
      %30 = arith.select %29, %in, %cst_0 : f32
      linalg.yield %30 : f32
    } -> tensor<1767x16xf32, #SPARSE>
    %10 = linalg.matmul ins(%arg1, %9 : tensor<1767x1767xf32, #SPARSE>, tensor<1767x16xf32, #SPARSE>) outs(%5 : tensor<1767x16xf32, #SPARSE>) -> tensor<1767x16xf32, #SPARSE>
    %11 = tensor.empty() : tensor<16x121xf32>
    %12 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel"]} ins(%cst_2 : tensor<121x16xf32, #SPARSE>) outs(%11 : tensor<16x121xf32>) {
    ^bb0(%in: f32, %out: f32):
      linalg.yield %in : f32
    } -> tensor<16x121xf32>
    ...
    %15 = linalg.matmul ins(%10, %12 : tensor<1767x16xf32, #SPARSE>, tensor<16x121xf32>) outs(%14 : tensor<1767x121xf32, #SPARSE>) -> tensor<1767x121xf32, #SPARSE>
    ...
    return %10 : tensor<1767x16xf32, #SPARSE>
  }
}