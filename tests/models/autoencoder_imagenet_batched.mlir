...
// ImageNet Batched Autoencoder: batch=32, 50176 (224x224x1 grayscale) -> 256 -> 50176
// Uses SpMM (Sparse Matrix-Matrix multiply) instead of SpMV
module attributes {torch.debug_module_name = "SparseAutoencoder_ImageNet_Batched"} {
  ml_program.global private mutable @global_seed(dense<0> : tensor<i64>) : tensor<i64>
  func.func @forward(%arg0: tensor<32x50176xf32, #SPARSE>) -> tensor<32x50176xf32, #SPARSE> {
    ...
    %cst_decoder_weight = arith.constant dense_resource<__elided__> : tensor<256x50176xf32, #SPARSE>
    ...
    // ===== ENCODER =====
    // Encoder: X @ W_enc -> (32, 50176) @ (50176, 256) = (32, 256)
    ...
    } ins(%arg0, %cst_encoder_weight : tensor<32x50176xf32, #SPARSE>, tensor<50176x256xf32, #SPARSE>)
    ...
    // ===== DECODER =====
    // Decoder: H @ W_dec -> (32, 256) @ (256, 50176) = (32, 50176)
    // W_dec is already stored as (256 x 50176), no transpose needed
    %dec_out_empty = tensor.empty() : tensor<32x50176xf32, #SPARSE>
    %dec_out_init = linalg.fill ins(%cst_0 : f32) outs(%dec_out_empty : tensor<32x50176xf32, #SPARSE>) -> tensor<32x50176xf32, #SPARSE>
    %dec_matmul = linalg.generic {
      indexing_maps = [#map3, #map4, #map5],
      iterator_types = ["parallel", "parallel", "reduction"]
    } ins(%enc_relu, %cst_decoder_weight : tensor<32x256xf32, #SPARSE>, tensor<256x50176xf32, #SPARSE>)
      outs(%dec_out_init : tensor<32x50176xf32, #SPARSE>) {
    ...
    %dec_biased = linalg.generic {
      indexing_maps = [#map, #map2, #map],
      iterator_types = ["parallel", "parallel"]
    } ins(%dec_matmul, %cst_decoder_bias : tensor<32x50176xf32, #SPARSE>, tensor<50176xf32>)
