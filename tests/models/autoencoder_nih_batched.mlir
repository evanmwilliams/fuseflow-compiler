...
// NIH Chest X-ray Batched Autoencoder: batch=32, 1048576 (1024x1024 grayscale) -> 512 -> 1048576
// Uses SpMM (Sparse Matrix-Matrix multiply) instead of SpMV
module attributes {torch.debug_module_name = "SparseAutoencoder_NIH_Batched"} {
  ml_program.global private mutable @global_seed(dense<0> : tensor<i64>) : tensor<i64>
  func.func @forward(%arg0: tensor<32x1048576xf32, #SPARSE>) -> tensor<32x1048576xf32, #SPARSE> {
    ...
    %cst_encoder_weight = arith.constant dense_resource<__elided__> : tensor<1048576x512xf32, #SPARSE>

    // ===== ENCODER =====
    // Encoder: X @ W_enc -> (32, 1048576) @ (1048576, 512) = (32, 512)
    ...
    } ins(%arg0, %cst_encoder_weight : tensor<32x1048576xf32, #SPARSE>, tensor<1048576x512xf32, #SPARSE>)
    ...
    // ===== DECODER =====
    // Decoder: H @ W_dec -> (32, 512) @ (512, 1048576) = (32, 1048576)
    // W_dec is already stored as (512 x 1048576), no transpose needed
    ...
    %dec_matmul = linalg.generic {
      indexing_maps = [#map3, #map4, #map5],
      iterator_types = ["parallel", "parallel", "reduction"]
    } ins(%enc_relu, %cst_decoder_weight : tensor<32x512xf32, #SPARSE>, tensor<512x1048576xf32, #SPARSE>)
