...
// Unfused 1: Encoder SpMM only
// Input: (32, 50176), Weight: (50176, 256) -> Output: (32, 256)
module attributes {torch.debug_module_name = "Unfused_1_Enc_SpMM"} {
  ...
  func.func @forward(%input: tensor<32x50176xf32, #SPARSE>) -> tensor<32x256xf32, #SPARSE> {
    ...
    %out_empty = tensor.empty() : tensor<32x256xf32, #SPARSE>
    %out_init = linalg.fill ins(%cst_0 : f32) outs(%out_empty : tensor<32x256xf32, #SPARSE>) -> tensor<32x256xf32, #SPARSE>
    %result = linalg.generic {
      indexing_maps = [#map, #map1, #map2],
      iterator_types = ["parallel", "parallel", "reduction"]
    } ins(%input, %cst_encoder_weight : tensor<32x50176xf32, #SPARSE>, tensor<50176x256xf32, #SPARSE>)
      outs(%out_init : tensor<32x256xf32, #SPARSE>) {
